...
def test_assemble_ordered(populated):
    ...
    assert(df.shape == (40, 41))
    expected = ['f{}_{}'.format(order[i][1], j) for i in xrange(4) for j in xrange(10)]
    expected.append('key')
    assert(set(df.columns) == set(expected))
...
def test_assemble_filtered_cgroup(populated_local, keys):
    ...
    expected = ['f{}_{}'.format(i, j) for i in [0, 3] for j in xrange(10)]
    expected.append('key')
    assert(set(df.columns) == set(expected))
...
def test_assemble_filtered_rgroup(populated_local, keys):
    ...
    expected = ['f{}_{}'.format(i, j) for i in range(4) for j in xrange(10)]
    expected.append('key')
    assert(set(df.columns) == set(expected))
...
def test_assemble_cgroup_args(populated_local, keys):
    ...
    assert(set(df.columns) == set(expected))
    assert((df.key == keys).all())
    for col in ['f0_{}'.format(i) for i in xrange(10)]:
